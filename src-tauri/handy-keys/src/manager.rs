...
use crate::types::{Hotkey, HotkeyEvent, HotkeyId, HotkeyState, KeyEvent, Key};
    ...
    fn process_event(&mut self, event: &KeyEvent) -> Vec<HotkeyEvent> {
        ...
        if event.is_key_down {
            ...
            let to_press: Vec<HotkeyId> = self
                ...
                .iter()
                .filter(|(&id, hotkey)| {
                    let key_matches = hotkey.key == event.key;
                    let modifiers_match = if event.key == Some(Key::F20) {
                        true
                    } else {
                        hotkey.modifiers.matches(event.modifiers)
                    };
                    key_matches && modifiers_match && !self.pressed_hotkeys.contains(&id)
                })
                .map(|(&id, _)| id)
                .collect();
...
mod tests {
    ...
    mod manager_state {
        ...
        #[test]
        fn f20_heartbeat_ignores_modifiers() {
            let mut state = ManagerState::new();
            let hotkey = Hotkey::new(Modifiers::empty(), Key::F20).unwrap();
            let id = HotkeyId(0);
            state.hotkeys.insert(id, hotkey);

            // Press F20 with Modifiers::CTRL_LEFT (user holding Ctrl during heartbeat)
            let event = make_key_event(Modifiers::CTRL_LEFT, Some(Key::F20), true);
            let results = state.process_event(&event);
            ...
            // Release F20 with Modifiers::CTRL_LEFT
            let event = make_key_event(Modifiers::CTRL_LEFT, Some(Key::F20), false);
            let results = state.process_event(&event);
