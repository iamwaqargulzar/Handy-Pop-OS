...
#[specta::specta]
#[cfg(target_os = "linux")]
pub fn change_update_checks_setting(app: AppHandle, _enabled: bool) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
...
#[tauri::command]
#[specta::specta]
pub fn change_output_volume_reduction_setting(
    app: AppHandle,
    percentage: u8,
) -> Result<(), String> {
    let mut settings = settings::get_settings(&app);
    settings.output_volume_reduction_percent = percentage.min(100);
    settings::write_settings(&app, settings);
    Ok(())
}
