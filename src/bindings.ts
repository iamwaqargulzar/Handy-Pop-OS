...
async changePostProcessReasoningEffortSetting(providerId: string, reasoningEffort: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_post_process_reasoning_effort_setting", { providerId, reasoningEffort }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async changeOutputVolumeReductionSetting(percentage: number) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_output_volume_reduction_setting", { percentage }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
output_volume_reduction_percent?: number; append_trailing_space?: boolean; app_language?: string; theme?: Theme; experimental_enabled?: boolean; lazy_stream_close?: boolean; keyboard_implementation?: KeyboardImplementation; show_tray_icon?: boolean; paste_delay_ms?: number; paste_delay_after_ms?: number; 
/**
 ...
 */
"TranscribeCpp" | "Parakeet" | "Moonshine" | "MoonshineStreaming" | "SenseVoice" | "GigaAM" | "Canary" | "Cohere" | "OpenVinoNpu"
...
/**
 * A pinned multi-file OpenVINO IR snapshot. This resolves to a directory
 * consumed by the isolated Intel NPU worker.
 */
